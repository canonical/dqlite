...
TEST(vfs2_write, oomPage, setUp, tearDown, 0, NULL)
{
	struct fixture *f = data;
	sqlite3_file *file = __file_create_main_db(f);
	void *buf_header_main = __buf_header_main_db();
 ...
 * the content object. */
TEST(vfs2_write, oomPageArray, setUp, tearDown, 0, NULL)
{
	struct fixture *f = data;
	sqlite3_file *file = __file_create_main_db(f);
	void *buf_header_main = __buf_header_main_db();
...
TEST(vfs2_write, oomPageBuf, setUp, tearDown, 0, NULL)
{
	struct fixture *f = data;
	sqlite3_file *file = __file_create_main_db(f);
	void *buf_header_main = __buf_header_main_db();
...
SUITE(vfs2_current_time)

TEST(vfs2_current_time, success, setUp, tearDown, 0, NULL)
{
	struct fixture *f = data;
	double now;
	int rc;
...
SUITE(vfs2_integration)

/* Test our expections on the memory-mapped WAl index format. */
TEST(vfs2_integration, wal, setUp, tearDown, 0, NULL)
{
	sqlite3 *db1;
	sqlite3 *db2;
	uint32_t *read_marks;
...
TEST(vfs2_integration, checkpoint, setUp, tearDown, 0, NULL)
{
	sqlite3 *db1;
	sqlite3 *db2;
