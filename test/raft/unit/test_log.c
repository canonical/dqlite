...
#define APPEND(TERM)                                              \
    {                                                             \
        ...
        rv_ = logAppend(f->log, TERM, RAFT_COMMAND, buf_, (struct raft_entry_local_data){}, true, NULL); \
    ...
    {                                                             \
        ...
        for (i = 0; i < N; i++) {                                 \
            ...
            *(uint64_t *)buf.base = i * 1000;                     \
            rv = logAppend(f->log, 1, RAFT_COMMAND, buf, (struct raft_entry_local_data){}, true, batch); \
...
TEST(logAppend, oom, setUp, tearDown, 0, logAppendOom)
{
    ...
    rv = logAppend(f->log, 1, RAFT_COMMAND, buf, (struct raft_entry_local_data){}, true, NULL);
...
TEST(logTruncate, acquiredOom, setUp, tearDown, 0, logTruncateAcquiredOom)
{
    ...
    rv = logAppend(f->log, 2, RAFT_COMMAND, buf, (struct raft_entry_local_data){}, true, NULL);
    munit_assert_int(rv, ==, RAFT_NOMEM);
