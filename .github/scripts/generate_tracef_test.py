...
""")
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        for future in as_completed(futures):
            ...
            remaining = len([f for f in futures.values() if f not in [s for s, _ in [(futures[f], f.done()) for f in futures if f.done()]]])
            print(f"Processing {source}... {remaining} remaining.")
    ...
    with open("debug_trace_generated.c", "a") as output:
