				  ...
				  struct client_context *context)
{
	...
	if (server->is_new && server->bootstrap) {
		rv = clientOpenAndHandshake(&server->proto, server->local_addr,
		...
		rv = clientConnectToSomeServer(&server->proto,
					       server->cache.nodes,
					       server->cache.len, context);
		if (rv != 0) {
			return 1;
		}

		rv = clientTryReconnectToLeader(&server->proto, context);
		if (rv != 0) {
			return 1;
		}
...
static void *refreshTask(void *arg)
{
	...
	for (;;) {
		...
		if (server->proto.fd == -1) {
			rv = clientConnectToSomeServer(
			    &server->proto, server->cache.nodes,
			    server->cache.len, &context);
			if (rv != 0) {
				continue;
			}
			(void)clientTryReconnectToLeader(&server->proto,
							 &context);
			if (server->proto.fd == -1) {
				continue;
			}
