...
/* Bind a parameter. */
static int bind_param(struct stmt *s, int n, struct value *value)
{
	int rc;
	...
	switch (value->type) {
		case SQLITE_INTEGER:
			rc = sqlite3_bind_int64(s->stmt, n, value->integer);
			break;
		case SQLITE_FLOAT:
			rc = sqlite3_bind_double(s->stmt, n, value->float_);
			break;
		case SQLITE_BLOB:
			assert(0); /* TODO */
			break;
		case SQLITE_NULL:
			rc = sqlite3_bind_null(s->stmt, n);
			break;
		case SQLITE_TEXT:
			rc = sqlite3_bind_text(s->stmt, n, value->text, -1,
					       SQLITE_TRANSIENT);
			break;
		case DQLITE_ISO8601:
			rc = sqlite3_bind_text(s->stmt, n, value->text, -1,
			...
			break;
		case DQLITE_BOOLEAN:
			rc = sqlite3_bind_int64(s->stmt, n,
						value->boolean == 0 ? 0 : 1);
			break;
		default:
			rc = DQLITE_PROTO;
 ...
 *
 * TODO: find a better way to handle time types. */
static int value_type(struct stmt *s, int i)
{
	int type = sqlite3_column_type(s->stmt, i);
	const char *column_type_name = sqlite3_column_decltype(s->stmt, i);
	if (column_type_name != NULL) {
		if (strcmp(column_type_name, "DATETIME") == 0) {
...
/* Append a single row to the message. */
static int encode_row(struct stmt *s, struct buffer *buffer, int n)
{
	...
	for (i = 0; i < n; i++) {
		/* Figure the type */
		struct value value;
		value.type = value_type(s, i);
		switch (value.type) {
			case SQLITE_INTEGER:
				value.integer =
				    sqlite3_column_int64(s->stmt, i);
				break;
			case SQLITE_FLOAT:
				value.float_ =
				    sqlite3_column_double(s->stmt, i);
				break;
			case SQLITE_BLOB:
				assert(0); /* TODO */
				break;
			...
			case SQLITE_TEXT:
				value.text =
				    (text_t)sqlite3_column_text(s->stmt, i);
				break;
			case DQLITE_UNIXTIME:
				value.integer =
				    sqlite3_column_int64(s->stmt, i);
