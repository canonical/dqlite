 ...
 * using this VFS should only be used on the thread that opened it. The functions
 * below that operate on sqlite3_file objects created by this VFS should also
 * only be used on that thread.
 */
 ...
 *
 * Call this on the sqlite3_file object for the WAL (SQLITE_FCNTL_JOURNAL_POINTER).
 */
...
/**
 * Retrieve information about frames that the last write transaction appended to the WAL.
 *
 * Call this on the sqlite3_file object for the WAL (SQLITE_FCNTL_JOURNAL_POINTER).
 */
...
/**
 * Unhide a write transaction that's been committed in Raft, and release the write lock.
 *
 * Call this on the sqlite3_file object for the main file (SQLITE_FCNTL_FILE_POINTER).
 */
