...
static int dqlite__lifecycle_refcount[] = {0, /* DQLITE__LIFECYCLE_ERROR */
...
static void dqlite__lifecycle_check_type(int type) {
	...
	sprintf(dqlite__lifecycle_errmsg, "%s lifecycle leak: %d\n", name, refcount);
}
...
int dqlite__lifecycle_check(char **errmsg) {
	...
	for (type = 0;
	     dqlite__lifecycle_refcount[type] != DQLITE__LIFECYCLE_REFCOUNT_NULL;
	     type++) {
