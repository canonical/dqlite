...
static size_t vfsWalSize(struct vfsWal *w)
{
	uint64_t size = 0;
	if (w->n_frames > 0) {
		uint32_t page_size;
...
static int vfsCommitShm(struct vfsMainFile *f) {
	...
	if (f->exclMask & (1 << VFS__WAL_CKPT_LOCK)) {
		...
		int32_t *sharedBackfill               = (first_region_shared + nBackfillOffset);
		*sharedBackfill                       = *privateBackfill;
...
{
	struct vfsMainFile *f = (struct vfsMainFile*)file;
	const uint16_t mask = (uint16_t)((1<<(ofst+n)) - (1<<ofst)); /* Mask of locks to take or release */
	...
	int rv = SQLITE_OK;
	if( ((flags & SQLITE_SHM_UNLOCK) && ((f->exclMask|f->sharedMask) & mask))
		...
		if (flags & SQLITE_SHM_UNLOCK) {
			...
			* If there is, just return SQLITE_BUSY. */
			// FIXME: this check is strange because we need to support
			// a weird way to check for checkpointing.
			if (ofst == VFS__WAL_WRITE_LOCK && f->database->wal.n_tx > 0) {
				rv = SQLITE_BUSY;
			} else {
				mtx_lock(&f->database->lock);
				rv = vfsShmLock(&f->database->shm, ofst, n, true);
				mtx_unlock(&f->database->lock);
				if (rv == SQLITE_OK) {
					f->exclMask |= mask;
				}
...
static int vfsDiskFileClose(sqlite3_file *file)
{
	...
	if (f->underlying != NULL) {
		int rv = f->underlying->pMethods->xClose(f->underlying);
...
static const sqlite3_io_methods vfsDiskFileMethods = {
	...
	/* TODO does reserved lock need to be treated differently in disk-mode? */
    .xCheckReservedLock     = vfsNoopCheckReservedLock,
		...
		unsigned *n)
{
	...
	*frames = sqlite3_malloc64(sizeof **frames * f->database->wal.n_tx);
	if (*frames == NULL) {
		return SQLITE_NOMEM;
	}
	*n = f->database->wal.n_tx;

	for (i = 0; i < f->database->wal.n_tx; i++) {
		dqlite_vfs_frame *frame = &(*frames)[i];
		uint32_t page_number = vfsFrameGetPageNumber(f->database->wal.tx[i]);
		frame->data = f->database->wal.tx[i]->page;
		 ...
		 * caller. */
		/* FIXME(marco6): it should be possible now not to free frames 
		 * and keep responsibility over this memory, so that when 
		 * publishing we can avoid recomputing the header. Not sure
		 * if it is worth it. */
...
static void vfsInvalidateWalIndexHeader(struct vfsDatabase *d)
{
	...
	ssize_t rv = write(shm->fd, buffer, 1);
	assert(rv == 1);

	buffer[0] = 0;
	lseek(shm->fd, VFS__WAL_INDEX_HEADER_SIZE, SEEK_SET);
	rv = write(shm->fd, buffer, 1);
	assert(rv == 1);
}
		...
		void *frames)
{
	...
	rv = vfsWalAppend(f->database, f->database->n_pages, n, page_numbers, frames);
	if (rv != 0) {
...
int VfsAbort(sqlite3 *conn)
{
	...
	if (f->exclMask & (1 << VFS__WAL_WRITE_LOCK)) {
		...
		rv = vfsShmUnlock(&f->database->shm, VFS__WAL_WRITE_LOCK, 1, true);
		if (rv == SQLITE_OK) {
...
int VfsSnapshot(sqlite3_vfs *vfs, const char *filename, void **data, size_t *n)
{
	...
	*n = (size_t)vfsDatabaseFileSize(database) + (size_t)vfsWalSize(wal);
	       ...
	       size_t n)
{
	...
	/* Lock the database. The locking scheme here is similar to the one used when
	 * transitioning from WAL to DELETE mode. The WAL-Index recovery is not enough
	 * as it lets connections read the content of the main file. This means that:
	 *  - all WAL-Index locks must be held exclusively, including READ(0).
	 *  - an exclusive lock must be held on the main file. 
	 * Given that only WAL mode is supported, a lock on the database object should
	 * be enough to emulate the file lock. */
	...
	if (rv != 0) {
		/* FIXME: the issue here is that the logic was able to restore the
		 * database, but not the WAL. This results in corrupted reads as
		 * the pages in the WAL might refer to a different version of the
		 * database. I wonder if it would make sense to at least reset the
		 * WAL so that it is still possible to read an old version of the
		 * database. Regardless, with the new "shallow" version of the
		 * snapshot mechanism, this is not a problem as the WAL segment
		 * is always empty. */
