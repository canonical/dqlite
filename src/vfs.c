...
{
	dqlite_assert((((uintptr_t)data) % sizeof(uint32_t)) == 0);
...
{
	dqlite_assert(f != NULL);
	dqlite_assert(f->page != NULL);
			      ...
			      void **page)
{
	...
	dqlite_assert(d != NULL);
	dqlite_assert(pgno > 0);
...
struct vfs
{
	const struct vfsConfig *config; /* Database configuration */
...
static struct vfs *vfsCreate(const struct vfsConfig *config)
{
	struct vfs *v;
					     ...
					     const char *filename)
{
	...
	dqlite_assert(v != NULL);
	dqlite_assert(filename != NULL);
...
static int vfsWalFileRead(sqlite3_file* file, void* buf, int amount, sqlite3_int64 offset)
{
	...
	if (offset == 0) {
		/* Read the header. */
		dqlite_assert(amount == VFS__WAL_HEADER_SIZE);
	...
	if (amount == FORMAT__WAL_FRAME_HDR_SIZE) {
		...
		dqlite_assert(((offset - 16 - VFS__WAL_HEADER_SIZE) %
			((int)page_size + FORMAT__WAL_FRAME_HDR_SIZE)) == 0);
		index =
		...
		dqlite_assert(amount == (FORMAT__WAL_FRAME_HDR_SIZE + (int)page_size));
		index =
		    (unsigned)formatWalCalcFrameIndex((int)page_size, offset);
	}
...
static int vfsMainFileClose(sqlite3_file *file)
{
	...
	if (f->database->wal.n_frames == 0) {
		dqlite_assert(f->database->n_pages > 0);
			   ...
			   sqlite_int64 offset)
{
	...
	if (offset < page_size) {
		...
		dqlite_assert((offset % page_size) == 0);
			    ...
			    sqlite_int64 offset)
{
	...
	if (offset == 0) {
		...
		dqlite_assert(amount >= FORMAT__DB_HDR_SIZE);
		 ...
		 * of the page size. */
		dqlite_assert((offset % (int)page_size) == 0);
...
static int vfsPragmaPageSize(struct vfsMainFile *f, char **fcntl)
{
	...
	unsigned page_size = (unsigned)atoi(fcntl[2]);
	if (page_size != f->vfs->config->page_size) {
		fcntl[0] = sqlite3_mprintf(
...
static void vfsForgeDeleteTransaction(struct vfsMainFile *f)
{
	...
	dqlite_assert(page_size > header_size);
	...
	if (
		...
		if (rv == SQLITE_OK && ofst == VFS__WAL_WRITE_LOCK) {
			if (flags == (SQLITE_SHM_LOCK | SQLITE_SHM_EXCLUSIVE)) {
				dqlite_assert(f->database->wal.n_tx == 0);
			}
		}
	}
		   ...
		   int *out_flags)
{
	...
	dqlite_assert(vfs->pAppData != NULL);
	dqlite_assert(file != NULL);
	 ...
	 */
	dqlite_assert(!exclusive || create);
	...
	dqlite_assert((flags & SQLITE_OPEN_DELETEONCLOSE) == 0);
	...
	dqlite_assert(flags & SQLITE_OPEN_MAIN_DB);
...
static int vfsDelete(sqlite3_vfs *vfs, const char *filename, int dir_sync)
{
	...
	dqlite_assert(false && "impossible");
	return SQLITE_PROTOCOL;
}
...
static int vfsAuthorizer(void *pUserData, int action, const char *third, const char *fourth, const char *fifth, const char *sixth) {
	(void)pUserData;
	(void)fourth;
	(void)fifth;
	(void)sixth;
	...
	if (action == SQLITE_ATTACH) {
		...
		struct {
			const char *name; /* The name of the disabled PRAGMA. */
			bool setter_only; /* `true` if only the setter is denied. */
		} denied_pragmas[] = {{
			.name = "journal_mode",
			.setter_only = true,
		}, {
		...
		for (size_t i = 0; i < (sizeof(denied_pragmas) / sizeof(denied_pragmas[0])); i++) {
			if (denied_pragmas[i].setter_only && !fourth) {
				continue;
			}
			if (sqlite3_stricmp(third, denied_pragmas[i].name) == 0) {
				return SQLITE_DENY;
			}
		}
	}
		       ...
		       const struct sqlite3_api_routines *pThunk)
{
	...
	/* Disable syncs. */
	rv = sqlite3_exec(db, "PRAGMA synchronous=OFF", NULL, NULL, pzErrMsg);
	if (rv != SQLITE_OK) {
	...
	if (f->database->n_pages == 0) {
		...
		/* Set WAL journaling. */
		rv = sqlite3_exec(db, "PRAGMA journal_mode=WAL", NULL, NULL,
				  pzErrMsg);
		if (rv != SQLITE_OK) {
		...
		rv = sqlite3_exec(db, "BEGIN IMMEDIATE; ROLLBACK", NULL, NULL,
				  pzErrMsg);
		if (rv != SQLITE_OK) {
	...
	rv = sqlite3_wal_autocheckpoint(db, 0);
	if (rv != SQLITE_OK) {
		tracef("sqlite3_wal_autocheckpoint off failed %d", rv);
		return rv;
	}
	...
	rv = sqlite3_db_config(db, SQLITE_DBCONFIG_ENABLE_FKEY, 1, NULL);
	if (rv != SQLITE_OK) {
		return rv;
	}

	sqlite3_set_authorizer(db, vfsAuthorizer, NULL);
	...
	const struct vfsTransaction *transaction)
{
	...
	for (i = 0; i < transaction->n_pages; i++) {
		...
		if (page_number == 1) {
			...
			if (database_size == 0) {
				dqlite_assert(transaction->n_pages == 1);
				database_size = 1;
			}
		}
...
int VfsCheckpoint(sqlite3 *conn)
{
	...
	return vfsCheckpoint(conn, (struct vfsMainFile*)file);
}
...
int VfsReleaseSnapshot(sqlite3 *conn, struct vfsSnapshot *snapshot)
{
	...
	if (snapshot->pages != f->database->pages) {
		dqlite_assert(f->database->wal.n_frames > 0);
...
uint64_t VfsDatabaseSize(sqlite3 *conn, unsigned n)
{
	...
	uint64_t new_wal_size = (uint64_t)vfsWalSize(&f->database->wal);
	if (new_wal_size == 0) {
		new_wal_size = (uint64_t)VFS__WAL_HEADER_SIZE;
	}
	new_wal_size += (uint64_t)n * (uint64_t)vfsFrameSize(f->vfs->config->page_size);
	return (uint64_t)vfsDatabaseFileSize(f->database) + new_wal_size;
}
