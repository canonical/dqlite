...
static void *vfsDatabasePageLookup(struct vfsContent *c, int pgno)
{
	struct vfsFrame *page;
	...
	switch (f->content->type) {
		...
		case VFS__WAL:
			...
			if (offset == 0) {
				...
				assert(f->content->wal.hdr != NULL);
				memcpy(buf, f->content->wal.hdr,
			...
			frame = vfsWalFrameLookup(f->content, pgno);
