...
int clientSendHandshake(struct client_proto *c, struct client_context *context)
{
	...
	if (rv < 0) {
		tracef("client send handshake failed %" PRIi64, (int64_t)rv);
		return DQLITE_CLIENT_PROTO_ERROR;
			...
			struct client_context *context)
{
	...
	if (rv < 0) {
		tracef("request write failed rv: %" PRIi64, (int64_t)rv);
		return DQLITE_CLIENT_PROTO_ERROR;
	} else if ((size_t)rv < n) {
		return DQLITE_CLIENT_PROTO_SHORT;
	}
		       ...
		       struct client_context *context)
{
	...
	if (rv != 0) {
		tracef("message decode failed rv: %" PRIi64, (int64_t)rv);
		return DQLITE_CLIENT_PROTO_ERROR;
	}
