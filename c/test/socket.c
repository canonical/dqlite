...
static int test_socket__server_bind_and_listen(struct test_socket__server *s) {
	...
	if (s->fd < 0) {
		test_suite_errorf("failed to open server socket: %s", strerror(errno));
		return -1;
	}

	err = bind(s->fd, address, size);
	if (err != 0) {
		test_suite_errorf("failed to bind server socket: %s", strerror(errno));
		return -1;
	}

	err = listen(s->fd, 1);
	if (err != 0) {
		test_suite_errorf("failed to listen server socket: %s", strerror(errno));
		return -1;
	}
	...
	if (err != 0) {
		test_suite_errorf("failed to get server address: %s", strerror(errno));
		return -1;
	}
...
static int test_socket__server_accept(struct test_socket__server *s) {
	...
	if (s->client_fd < 0) {
		test_suite_errorf("failed to accept client connection: %s", strerror(errno));
		return -1;
	}
	...
	if (err != 0) {
		test_suite_errorf("failed to set non-blocking mode on client connection: %s", strerror(errno));
		return -1;
	}
...
static int test_socket__client_connect(struct test_socket__client *c) {
	...
	if (c->fd < 0){
		test_suite_errorf("failed to open client socket: %s", strerror(errno));
		return -1;
	}
	...
	if (err != 0) {
		test_suite_errorf("failed to connect to server socket: %s", strerror(errno));
		return -1;
	}
...
int test_socket_pair_cleanup(struct test_socket_pair *p)
{
	...
	if (err != 0) {
		if (!p->client_disconnected || errno != EBADF) {
			test_suite_errorf("failed to close client socket: %s - %d", strerror(errno), errno);
			return -1;
		}
	}
	...
	if (err != 0) {
		if (!p->server_disconnected || errno != EBADF) {
			test_suite_errorf("failed to close server socket: %s - %d", strerror(errno), errno);
			return -1;
		}
	}
	...
	err = close(p->listen);
	if (err != 0) {
		test_suite_errorf("failed to close listen socket: %s", strerror(errno));
		return -1;
	}
...
int test_socket_pair_client_disconnect(struct test_socket_pair *p)
{
	...
	err = close(p->client);
	if (err != 0) {
		test_suite_errorf("failed to disconnect client: %s - %d", strerror(errno), errno);
		return -1;
	}
...
int test_socket_pair_server_disconnect(struct test_socket_pair *p)
{
	...
	err = close(p->server);
	if (err != 0) {
		test_suite_errorf("failed to disconnect server: %s - %d", strerror(errno), errno);
		return -1;
	}
