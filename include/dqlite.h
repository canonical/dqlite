...
/**
 * Configure the target number of voting nodes for this server's cluster.
 *
 * When this server is the cluster leader, it will try to promote and demote
 * other servers to reach the target voter count. You should configure the same
 * target on all servers that will participate in the cluster.
 */
int dqlite_server_set_target_voters(dqlite_server *server, int n);

/**
 * Configure the target number of standby nodes for this server's cluster.
 *
 * When this server is the cluster leader, it will try to promote and demote
 * other servers to reach the target standby count. You should configure the
 * same target on all servers that will participate in the cluster.
 */
